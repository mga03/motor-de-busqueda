...
export class AppService {
  ...
  async getFilters(templateId: string = 'icons_hpov') {
    const url = `http://localhost:5984/service_type/${templateId}`;
    ...
    if (!resp.ok) {
      ...
      return { 
        history: { 
          filters: [
            { label: 'Nombre', attrib: 'ID' },
            { label: 'Equipamiento', attrib: 'meta.equipment' }
          ] 
        } 
      };
    }
    const doc = (await resp.json()) as any;
    // La estructura real es: { history: { filters: [...] } }
    const filters = doc?.history?.filters;
    if (!Array.isArray(filters) || filters.length === 0) {
      this.logger.warn(`CouchDB document missing history.filters array. Received: ${JSON.stringify(doc?.history)}, falling back to defaults.`);
      return { 
        history: { 
          filters: [
            { label: 'Nombre', attrib: 'ID' },
    ...
    this.logger.log(`Loaded ${filters.length} filters from CouchDB: ${filters.map((f: any) => f.label || f.attrib).join(', ')}`);
  ...
  buildSearchQuery(filters: Record<string, any>, size = 20, searchAfter?: string[]) {
    ...
    // Dynamic filters (meta fields) - always prefix with 'meta.' if not already present
  ...
  async setupIndexAndSample(templateId: string, projectName: string) {
    ...
    if (exists.ok) {
      await fetch(indexUrl, { method: 'DELETE', headers });
    }
